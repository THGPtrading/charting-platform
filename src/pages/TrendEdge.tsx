...
const TrendEdge: React.FC = () => {
  ...
  const renderRows = (label: string, candles: ChartCandle[]) => {
    return candles.slice(0, 6).map((candle, i) => (
      <div key={i} onClick={() => {
        try { (window as any).__chartSync?.publishHighlight?.('trendedge', candle.time as any); } catch {}
      }} style={{
        padding: "0.5rem",
        backgroundColor: i % 2 === 0 ? "#1e1e1e" : "#2a2a2a",
        color: "#e0e0e0",
      }}>
        {`${label} | O:${candle.open} H:${candle.high} L:${candle.low} C:${candle.close} | ${new Date(candle.time * 1000).toLocaleString()}`}
      </div>
    ));
  };

  // Strategy selector + signals
              ...
              <div style={{ flex: 1 }}>
                <ErrorBoundary>
                  <ChartHost
                    ...
                    showVolume={true}
                    rsiPeriods={[7,14,21]}
                    macdConfig={{ fast: 6, slow: 13, signal: 5 }}
                    showATR={false}
                  />
                </ErrorBoundary>
              </div>
